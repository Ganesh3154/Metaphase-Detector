...
const ViewImages = (props) => {
  ...
  useEffect(() => {
    ...
    axios
      .get(`http://localhost:8000/patient/images/${props.viewId}`)
      .then((res) => {
        console.log(res.data);
        setImgSrc(res.data);
        setMainImg(`http://127.0.0.1:8080/${props.viewId}/${res.data[0].path}`);
      });
  }, []);
              ...
              <>
                ...
                <div id={styles.thumbnails}>
                  ...
                  {imgSrc?.map((item, i) => (
                    <img
                      key={i}
                      src={`http://127.0.0.1:8080/${props.viewId}/${item.path}`}
                      onClick={(e) => {
                        changeImg(e);
                      }}
                    />
